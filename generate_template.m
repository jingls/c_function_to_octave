...
## -*- texinfo -*- 
## @deftypefn  {} {@var{retval} =} generate_template (@var{return_info}, @var{file_out}, @var{args}, @var{doc}, @var{code}, @var{prepend_name})
##
...
## @end table
##
## The arguments to @code{generate_template} are:
##
## @table @asis
...
function retval = generate_template (return_info, file_out, args, doc, code, prepend_name)
  
  str_out = ["\n\
// DO NOT EDIT! Created by generate_template.m\n\
...
// args(" num2str(i) ").has_underline : " num2str(args(i).has_underline) "\n\n\
"];
  endfor
  
  str_out = [str_out "\
// PKG_ADD: autoload (\"" prepend_name "OCTAVE_NAME\", which (\"" prepend_name "OCTAVE_CATEGORY\"));\n\
DEFUN_DLD(" prepend_name "OCTAVE_NAME, args, nargout, \"\\\n\
  -*- texinfo -*-\\n\\\n"];
  ...
  str_out = [str_out "@deftypefn {Loadable Function} {" outvars "} " prepend_name "OCTAVE_NAME ("];
...
"];
  ...
  free (arg" i_str ");\n"];
    ...
    " args(i).type " * arg" i_str " = &arg" i_str "_obj;\n"];
      ...
      str_inputs = [str_inputs "\n\
      Error!!!! Cannot proceed. Check generate_template.m\n"];
  ...
  str_call = [str_call prepend_name "FUNC_NAME ("];
  ...
  retval(" num2str(retval_pos) ") = octave_value (y" num2str(i) "_NDArray);\n"];
          ...
          str_return = [str_return "\n\
      Error!!!! Trying to convert type \"" args(i).type "\" to Octave's representation. Unkown conversion. Cannot proceed. Check generate_template.m\n"];
...
##      str_fmt = "%s"; % unknown type
      str_fmt = "unknown type. Please add format to function 'str_fmt' in file 'generate_template.m'. \""; % unknown type. Give an error
  endswitch
endfunction
